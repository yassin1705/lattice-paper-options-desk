...
import type { DecisionContext, RiskContext, RiskPolicy } from '@/lib/agents/types';
...
function contextId(scope: 'decision' | 'risk', subject: string, observedAt: string): string {
...
export class AgentDataCoordinator {
  ...
  async buildDecisionContext(request: DecisionContextRequest): Promise<DecisionContext> {
  ...
  async buildRiskContext(request: RiskContextRequest): Promise<RiskContext> {
    ...
    const [account, positions, openOrders, contract, snapshots, marketClock] = await Promise.all([
      this.alpaca.getAccount(),
      this.alpaca.getOpenPositions(),
      this.alpaca.getOrders({ status: 'open', assetClass: 'us_option', limit: 500 }),
    ...
    return {
      ...
      openOrders,
      contract,
      contractMarket: snapshots.find((snapshot) => snapshot.symbol === contractSymbol) ?? null,
      marketClock,
