...
import type { CompletedOptionTrade, OpenOptionPosition } from '@/lib/dashboard/types';
    ...
    <Card className="min-w-[280px] flex-1 border-white/8 bg-white/[0.025] shadow-none lg:min-w-[315px]">
        ...
        <div className="flex items-start justify-between gap-3">
          <div>
            <div className="flex items-center gap-2">
              <CardTitle className="text-lg text-white">{position.underlying}</CardTitle>
              <Badge
                variant="outline"
                className={cn(
            ...
            <p className="mt-1 truncate text-xs text-zinc-500">{position.underlyingName}</p>
          </div>
          <div
            className={cn(
              'flex items-center gap-1 rounded-lg px-2 py-1 text-xs font-semibold tabular-nums',
              positive ? 'bg-emerald-300/10 text-emerald-300' : 'bg-rose-300/10 text-rose-300',
            )}
          >
      ...
      <CardContent className="space-y-4">
        <div className="grid grid-cols-3 gap-3 border-y border-white/6 py-3">
          <PositionMetric label="Strike" value={money.format(position.strike)} />
          <PositionMetric label="Contracts" value={String(position.quantity)} />
          <PositionMetric label="Mark" value={money.format(position.currentPrice)} />
        </div>
        <div className="flex items-end justify-between gap-4">
          <div>
            <p className="text-[10px] uppercase tracking-[0.14em] text-zinc-600">Market value</p>
          ...
          <div className="text-right">
            <p className="flex items-center justify-end gap-1 text-[10px] uppercase tracking-[0.14em] text-zinc-600">
              <CalendarClock className="size-3" /> {position.daysToExpiration}d left
            </p>
...
function PositionMetric({ label, value }: { label: string; value: string }) {
  return (
    <div>
      ...
      <p className="mt-1 text-sm font-medium tabular-nums text-zinc-200">{value}</p>
    </div>
  );
}
      ...
      <DrawerContent className="border-white/10 bg-[#0c1512]">
        ...
        <div className="flex-1 space-y-6 overflow-y-auto p-6">
          <div className="grid grid-cols-2 gap-4">
            ...
            <PositionMetric
              label="Exit price"
              value={trade.exitPrice === null ? 'Unavailable' : money.format(trade.exitPrice)}
            />
          </div>
          <div className="rounded-xl border border-white/8 bg-white/[0.025] p-4">
            <p className="text-[10px] uppercase tracking-[0.14em] text-zinc-600">Execution report</p>
            <p className="mt-3 text-sm leading-6 text-zinc-300">{trade.report}</p>
          </div>
          <div className="flex gap-3 rounded-xl border border-emerald-300/10 bg-emerald-300/[0.04] p-4">
            ...
            <p className="text-xs leading-5 text-zinc-400">
              This view explains recorded activity only. It cannot submit, replace, or cancel orders.
            </p>
          </div>
        </div>
        <DrawerFooter className="border-t border-white/7 p-6">
          <DrawerClose render={<Button variant="outline">Close report</Button>} />
        </DrawerFooter>
      </DrawerContent>
    </Drawer>
  );
    ...
    return (
      <div className="grid min-h-64 place-items-center text-center">
        <div>
          ...
          <p className="mt-3 text-sm font-medium text-zinc-300">No completed trades yet</p>
          <p className="mt-1 text-xs text-zinc-600">Closed option orders will appear here.</p>
        </div>
      </div>
    );
  }
            ...
            return (
              <TableRow key={trade.id} className="border-white/6 hover:bg-white/[0.025]">
                ...
                <TableCell>
                  ...
                  <p className="mt-0.5 max-w-48 truncate text-[11px] text-zinc-600">{trade.contract}</p>
                </TableCell>
                <TableCell className="text-right tabular-nums text-zinc-400">{trade.quantity}</TableCell>
                <TableCell className="text-right tabular-nums text-zinc-400">
                  {trade.exitPrice === null ? '—' : money.format(trade.exitPrice)}
                </TableCell>
                <TableCell
                  className={cn(
                ...
                <TableCell>
                  <Badge variant="outline" className="border-white/8 bg-white/[0.03] capitalize text-zinc-400">
                    {trade.status}
                  </Badge>
                </TableCell>
    ...
    <Card className="flex min-h-[470px] flex-col border-white/8 bg-white/[0.025] shadow-none">
      <CardHeader className="border-b border-white/7">
        <div className="flex items-center gap-3">
          ...
          <div>
            <CardTitle className="text-sm text-white">Explainability chat</CardTitle>
            <p className="mt-1 text-[11px] text-zinc-600">LLM layer · coming later</p>
          </div>
        </div>
      </CardHeader>
      <CardContent className="flex flex-1 flex-col p-4">
        <div className="flex flex-1 items-center justify-center px-5 text-center">
          <div>
            ...
            <p className="mt-3 text-sm font-medium text-zinc-400">Conversation is not enabled</p>
            <p className="mt-1 text-xs leading-5 text-zinc-600">
              This space will explain decisions, risk checks, and completed trade reports.
            </p>
          </div>
        </div>
        <div className="relative mt-4">
          ...
          />
          <Button disabled size="icon" className="absolute bottom-2 right-2 size-8">
    ...
    <AppShell active="trades" connection={snapshot.connection}>
      <section className="mb-6 flex flex-col justify-between gap-3 sm:flex-row sm:items-end">
        <div>
          ...
          <h1 className="mt-2 text-2xl font-semibold tracking-tight text-white sm:text-3xl">Options activity</h1>
          <p className="mt-2 max-w-2xl text-sm text-zinc-500">
            Open positions and completed execution history from the paper account.
          </p>
        </div>
      ...
      <section aria-labelledby="open-positions-heading">
        <div className="mb-3 flex items-center justify-between">
          <h2 id="open-positions-heading" className="text-sm font-medium text-zinc-200">
            Open option positions
          </h2>
          <span className="text-xs tabular-nums text-zinc-600">{snapshot.openPositions.length} active</span>
        </div>
        {loading ? (
          <div className="flex gap-4 overflow-hidden">
            {[0, 1, 2].map((item) => (
              <Skeleton key={item} className="h-60 min-w-[300px] flex-1 bg-white/5" />
            ))}
          </div>
          ...
          <Card className="grid min-h-44 place-items-center border-dashed border-white/8 bg-transparent shadow-none">
            <div className="text-center">
              ...
              <p className="mt-3 text-sm text-zinc-400">No open option positions</p>
            </div>
          </Card>
        )}
      </section>
      ...
      <div className="mt-7 grid gap-5 xl:grid-cols-[minmax(0,1fr)_340px]">
        <Card className="overflow-hidden border-white/8 bg-white/[0.025] shadow-none">
          <CardHeader className="border-b border-white/7">
            <CardTitle className="text-sm text-white">Finished trades</CardTitle>
            <p className="text-xs text-zinc-600">Only terminal trade states are shown in this ledger.</p>
          </CardHeader>
          <CardContent className="p-0">
            {loading ? <Skeleton className="m-5 h-64 bg-white/5" /> : <CompletedTradesTable trades={snapshot.completedTrades} />}
          </CardContent>
        </Card>
        <ChatPlaceholder />
      </div>
